  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        ...
        <p className="text-[#555] text-sm mt-1">Gérez les services, prix et offres de lancement</p>
      </div>
      ...
      <div className="space-y-4">
        {services.map((s) => {
          ...
          return (
            <div
              key={s.id}
              className={`bg-dark-surface border p-6 transition-all ${
                !s.actif ? 'border-dark-border/40 opacity-60' : s.remise > 0 ? 'border-gold/20' : 'border-dark-border'
              }`}
            >
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
                ...
                <div className="lg:col-span-4 space-y-3">
                  <div>
                    <label className="text-xs text-[#555] uppercase tracking-wider block mb-1">Nom</label>
                    <input
                ...
                <div className="lg:col-span-2 space-y-3">
                  ...
                  <div>
                    <p className="text-xs text-[#555] uppercase tracking-wider mb-1">Marge</p>
                    <p className="font-cinzel text-xl font-bold text-gold">{marge}%</p>
                  </div>
                </div>

                {/* Toggle -20% */}
                <div className="lg:col-span-3">
                  <p className="text-xs text-[#555] uppercase tracking-wider mb-3">Offre de lancement</p>
                  <button
                    onClick={() => toggleRemise(s.id, s.remise)}
                    disabled={isSaving}
                    className={`relative w-full flex items-center justify-between px-4 py-3 border font-cinzel text-sm tracking-wider transition-all duration-300 ${
                      s.remise > 0
                        ? 'border-gold bg-gold/10 text-gold hover:bg-gold/20'
                        : 'border-dark-border text-[#555] hover:border-gold/40 hover:text-[#888]'
                    }`}
                  >
                    <div className="flex items-center gap-3">
                      {/* Toggle pill */}
                      <div className={`w-10 h-5 rounded-full border-2 transition-all duration-300 relative ${
                      ...
                      <span>{s.remise > 0 ? '-20% ACTIF' : '-20% inactif'}</span>
                    </div>
                    {isSaving ? (
                      <span className="text-xs opacity-50">...</span>
                    ) : s.remise > 0 ? (
                      <span className="text-xs text-gold/70">
                        {prixRemise?.toFixed(0)}€ affiché
                      </span>
                    ) : (
                      <span className="text-xs text-[#444]">{s.prixBase}€ affiché</span>
                    )}
                  </button>
                  {s.remise > 0 && (
                    <div className="mt-2 flex items-center gap-2 text-xs text-[#888]">
                      <span className="line-through text-[#444]">{s.prixBase}€</span>
                      <span className="text-gold">→</span>
                      <span className="text-gold font-semibold">{prixRemise?.toFixed(0)}€</span>
                ...
                <div className="lg:col-span-3 flex flex-col gap-3">
                  <div>
                    <p className="text-xs text-[#555] uppercase tracking-wider mb-2">Visibilité site</p>
                    <button
                      onClick={() => toggleActif(s.id, s.actif)}
                      disabled={isSaving}
                      className={`flex items-center gap-2 px-4 py-2 border text-xs font-cinzel tracking-wider transition-all ${
                        s.actif
                  ...
                  <button
                    ...
                    disabled={isSaving}
                    className={`py-2 font-cinzel text-sm font-semibold tracking-wider transition-all mt-auto ${
                      isSaved
